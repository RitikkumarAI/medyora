...
import { Search, ArrowLeft, Star, Clock, Heart, SlidersHorizontal, X, MapPin, Scale } from "lucide-react";
...
function DoctorsListing() {
  ...
  const filteredDoctors = useMemo(() => {
    return DOCTORS.filter(doc => {
      ...
      if (selectedArea && (!doc.clinic.area || !doc.clinic.area.toLowerCase().includes(selectedArea.toLowerCase()))) {
        return false;
      }
  ...
  }, [searchQuery, selectedCity, selectedArea, activeQuick, gender, maxFee, selectedLanguages, homeVisitOnly, onlineOnly]);
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center justify-between gap-3 mb-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-xl sm:text-2xl font-bold text-slate-900 dark:text-white leading-tight">Find Verified Doctors</h1>
                <p className="text-xs text-slate-500 dark:text-slate-400 font-medium">{filteredDoctors.length} specialists available</p>
              </div>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 w-full flex gap-6">
        ...
        <aside className="hidden lg:block w-72 shrink-0 space-y-6 bg-white dark:bg-slate-900 p-5 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-xs h-fit sticky top-28">
          ...
          <div className="space-y-2">
            <label className="text-xs font-bold uppercase tracking-wider text-slate-400">Doctor Gender</label>
          ...
          {/* Languages */}
          <div className="space-y-2">
            <label className="text-xs font-bold uppercase tracking-wider text-slate-400">Languages Spoken</label>
        ...
        {filteredDoctors.length === 0 ? (
          ...
          filteredDoctors.map((doc) => {
            ...
            return (
              <Link key={doc.id} to={`/doctors/$doctorId`} params={{ doctorId: doc.id }} className={`flex gap-4 rounded-3xl bg-white dark:bg-slate-900 p-3 shadow-xs border transition-colors relative overflow-hidden group ${isComparing ? "border-blue-600 bg-blue-50/10 dark:bg-blue-950/20" : "border-slate-100 dark:border-slate-800 hover:border-slate-300 dark:hover:border-slate-700"}`}>
                <img src={doc.image} alt={doc.fullName} className="h-24 w-24 rounded-2xl object-cover border border-slate-100 dark:border-slate-800 shrink-0" />
                <div className="flex flex-1 flex-col py-1 justify-center relative min-w-0">
                  ...
                  <div className="absolute top-0 right-0 flex items-center gap-1 z-10 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xs rounded-bl-xl pl-1 pb-1">
                    ...
                    <button 
                      onClick={(e) => { e.preventDefault(); toast.success("Added to favorites"); }} 
                  ...
                  <div className="pr-12">
                    <h4 className="font-bold text-[15px] text-slate-900 dark:text-white line-clamp-1">{doc.fullName}</h4>
                    <p className="text-xs font-medium text-blue-600 dark:text-blue-400 mt-0.5">{doc.speciality}</p>
                  </div>
                  ...
                  <div className="flex items-center justify-between mt-3">
                    ...
                    <div className="font-black text-slate-900 dark:text-white text-[15px]">₹{doc.fee}</div>
                  </div>
                </div>
              </Link>
            );
      ...
      {compareList.length > 0 && (
        <div className="fixed bottom-0 left-0 right-0 p-4 bg-white dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800 pb-safe shadow-2xl z-40">
          <div className="flex items-center justify-between mb-3">
            <span className="text-sm font-bold text-slate-700 dark:text-slate-200">{compareList.length} / 4 Selected</span>
            <button onClick={() => setCompareList([])} className="text-xs font-bold text-slate-400 hover:text-slate-700 dark:hover:text-slate-200">Clear</button>
          </div>
          <Button asChild disabled={compareList.length < 2} className="w-full h-14 rounded-2xl text-[15px] font-bold bg-blue-600 hover:bg-blue-700 shadow-lg shadow-blue-600/25">
        ...
        <div className="fixed inset-0 z-50 flex items-end justify-center sm:items-center bg-slate-900/50 backdrop-blur-xs transition-opacity">
          <div className="w-full sm:max-w-md bg-white dark:bg-slate-900 rounded-t-[32px] sm:rounded-[32px] shadow-2xl overflow-hidden flex flex-col max-h-[90vh] border border-slate-200 dark:border-slate-800">
            <div className="px-6 py-4 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between sticky top-0 bg-white dark:bg-slate-900 z-10">
              ...
              <Button variant="ghost" size="icon" onClick={() => setShowFilters(false)} className="h-8 w-8 rounded-full bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300">
                <X className="h-4 w-4" />
              </Button>
            </div>
            ...
            <div className="p-6 overflow-y-auto space-y-8 flex-1">
                  ...
                  {['any', 'male', 'female'].map((g) => (
                    <button
                      ...
                      className={`flex-1 py-2 text-sm font-bold rounded-xl capitalize transition-all ${
                        gender === g ? "bg-white dark:bg-slate-900 text-blue-600 dark:text-blue-400 shadow-xs" : "text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200"
                      }`}
                    >
                      {g}
                    </button>
              ...
              {/* Languages */}
              <div>
                <h3 className="text-sm font-bold text-slate-900 dark:text-white mb-3">Languages Spoken</h3>
              ...
              {/* Preferences */}
              <div>
                <h3 className="text-sm font-bold text-slate-900 dark:text-white mb-3">Preferences</h3>
                <div className="space-y-3">
                  <label className="flex items-center justify-between p-4 rounded-2xl border border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/60 cursor-pointer">
                    <span className="text-sm font-bold text-slate-700 dark:text-slate-300">Home Visit Available</span>
                    <input type="checkbox" checked={homeVisitOnly} onChange={(e) => setHomeVisitOnly(e.target.checked)} className="w-5 h-5 rounded border-slate-300 text-blue-600 focus:ring-blue-600" />
                  </label>
                  <label className="flex items-center justify-between p-4 rounded-2xl border border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/60 cursor-pointer">
                    <span className="text-sm font-bold text-slate-700 dark:text-slate-300">Online Consultation</span>
                    <input type="checkbox" checked={onlineOnly} onChange={(e) => setOnlineOnly(e.target.checked)} className="w-5 h-5 rounded border-slate-300 text-blue-600 focus:ring-blue-600" />
                  </label>
                </div>
              </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-sm font-bold text-slate-900 dark:text-white">Max Consultation Fee</h3>
                  <span className="text-sm font-bold text-blue-600 dark:text-blue-400">₹{maxFee}</span>
                </div>
                <input 
                  type="range" 
            ...
            <div className="p-4 border-t border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 sticky bottom-0">
              <Button onClick={() => setShowFilters(false)} className="w-full h-14 rounded-2xl text-[15px] font-bold bg-blue-600 hover:bg-blue-700 shadow-lg shadow-blue-600/25">
