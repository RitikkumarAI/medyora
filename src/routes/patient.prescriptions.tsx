  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#F8FAFC] dark:bg-slate-950 text-slate-900 dark:text-slate-100 font-sans transition-colors pb-24">
      ...
      <main className="flex-1 px-4 py-4 space-y-6">
        ...
        {/* Prescriptions */}
        <div>
          <h2 className="text-sm font-bold text-slate-400 uppercase tracking-wider mb-4 px-2">Prescriptions</h2>
          <div className="space-y-4">
            {PRESCRIPTIONS.map((p) => (
              <div key={p.id} className="bg-white dark:bg-slate-900 rounded-[24px] p-5 shadow-xs border border-slate-100 dark:border-slate-800">
                <div className="flex items-start justify-between border-b border-slate-100 dark:border-slate-800 pb-4 mb-4">
                  <div>
                    ...
                    <p className="text-xs font-medium text-slate-500 dark:text-slate-400 mt-0.5">{p.clinic} • {p.date}</p>
                  </div>
                  <Button
                    variant="outline"
                    size="icon"
                ...
                <div className="space-y-3">
                  <div className="bg-[#F8FAFC] dark:bg-slate-800 rounded-2xl p-3">
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wide mb-1">Diagnosis</p>
                    <p className="text-sm font-medium text-slate-700 dark:text-slate-200">{p.diagnosis}</p>
                  </div>

                  <div className="space-y-2">
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wide px-1">Medicines</p>
                    {p.medicines.map((m, i) => (
                      <div key={i} className="flex items-center gap-3 p-3 rounded-2xl border border-slate-100 dark:border-slate-800">
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-xs text-slate-500 dark:text-slate-400">{m.dosage} • {m.duration}</p>
                        </div>
                      </div>
                    ))}
                  </div>
                  ...
                  {p.advice && (
                    <div className="bg-green-50 dark:bg-green-950/60 rounded-2xl p-3 mt-2">
                      <p className="text-[10px] font-bold text-green-600 dark:text-green-400 uppercase tracking-wide mb-1">Doctor's Advice</p>
                      <p className="text-sm font-medium text-green-900 dark:text-green-200">{p.advice}</p>
                    </div>
                  )}
                </div>
              </div>
        ...
        {/* Lab Reports */}
        <div>
          <h2 className="text-sm font-bold text-slate-400 uppercase tracking-wider mb-4 px-2">Lab Reports</h2>
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs font-medium text-slate-500 dark:text-slate-400 mt-0.5">{r.date} • {r.size}</p>
                    </div>
                  </div>
                  <Button
                    variant="ghost"
