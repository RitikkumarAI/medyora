...
import { 
  Users, UserCog, CalendarDays, IndianRupee, Bell, 
  CheckCircle, XCircle, FileText, Settings, ShieldCheck, ChevronRight 
} from "lucide-react";
...
export function AdminDashboard() {
  ...
  const gridItems = [
    ...
    { icon: Users, label: "Patients", link: "/admin/patients", color: "bg-emerald-100 text-emerald-600" },
    { icon: CalendarDays, label: "Appointments", link: "/admin/appointments", color: "bg-purple-100 text-purple-600" },
    { icon: IndianRupee, label: "Payments", link: "/admin/payments", color: "bg-amber-100 text-amber-600" },
    { icon: FileText, label: "CMS", link: "/admin/cms", color: "bg-rose-100 text-rose-600" },
    { icon: Settings, label: "Settings", link: "/admin/settings", color: "bg-slate-100 text-slate-600" },
  ];
    ...
    <div className="flex flex-col min-h-screen bg-[#F8FAFC] pb-24">
      ...
      <header className="bg-indigo-600 px-6 pt-12 pb-16 rounded-b-[40px] text-white shadow-md relative z-10">
        <div className="flex items-center justify-between mb-2">
          <div>
            <p className="text-indigo-100 text-xs font-medium uppercase tracking-wider">Super Admin</p>
      ...
      <main className="flex-1 px-5 -mt-8 relative z-20 space-y-6">
        ...
        <section>
          <div className="flex items-center justify-between mb-4 px-1">
            ...
            <Link to="/admin/doctors" className="text-xs font-bold text-indigo-600 flex items-center">View All <ChevronRight className="h-4 w-4" /></Link>
          </div>
          ...
          <div className="space-y-4">
            {isLoading ? (
              ...
              DOCTORS.slice(0, 2).map((doc) => (
                <div key={doc.id} className="bg-white p-4 rounded-[24px] border border-slate-100 shadow-sm flex flex-col gap-4">
                  <div className="flex gap-4 items-center">
                    <img src={doc.image} alt={doc.name} className="h-14 w-14 rounded-full object-cover border border-slate-100" />
                    <div>
                  ...
                  <div className="flex gap-2">
                    <Button variant="outline" className="flex-1 h-12 rounded-2xl border-red-100 bg-red-50 text-red-600 hover:bg-red-100 font-bold">
