...
import { ArrowLeft, FileText, Activity, History, Plus, Phone, MessageSquare, HeartPulse } from "lucide-react";
    ...
    <div className="flex flex-col min-h-screen bg-[#F8FAFC] pb-24">
      ...
      <header className="sticky top-0 z-40 bg-white px-4 pt-6 pb-2 shadow-sm border-b border-slate-100">
        ...
        <div className="flex items-center gap-4 py-2">
          <img src={patient.image} alt={patient.name} className="h-16 w-16 rounded-full object-cover border-2 border-slate-100" />
          <div>
            <h2 className="font-bold text-lg text-slate-900">{patient.name}</h2>
            <p className="text-xs font-medium text-slate-500 mt-1">{patient.age} yrs • {patient.gender} • {patient.bloodGroup}</p>
          </div>
        </div>
            ...
            <div className="bg-white p-4 rounded-3xl border border-slate-100 shadow-sm relative">
              ...
              <div className="mb-4">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">12 Aug 2026</p>
                </div>
                <h4 className="font-bold text-slate-900 text-[15px]">Viral Fever & Cough</h4>
                <p className="text-xs font-medium text-slate-600 mt-1 leading-relaxed">Patient complained of high fever (102F) and dry cough for 3 days. Prescribed Paracetamol and a cough syrup.</p>
              ...
              <div>
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">05 Jul 2026</p>
                </div>
                <h4 className="font-bold text-slate-900 text-[15px]">Routine Checkup</h4>
                <p className="text-xs font-medium text-slate-600 mt-1 leading-relaxed">BP normal (120/80). Prescribed Vitamin D supplements.</p>
              </div>
            </div>
          </div>
        )}
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="bg-white p-4 rounded-3xl border border-slate-100 shadow-sm flex flex-col items-center text-center gap-2">
                ...
                <p className="text-xl font-black text-slate-900">95 <span className="text-[10px] text-slate-400">mg/dL</span></p>
              </div>
            </div>
          </div>
        )}
      ...
      <Button asChild className="fixed bottom-6 left-1/2 -translate-x-1/2 h-14 rounded-full bg-blue-600 text-white font-bold px-6 shadow-xl shadow-blue-600/30 z-50 hover:bg-blue-700">
