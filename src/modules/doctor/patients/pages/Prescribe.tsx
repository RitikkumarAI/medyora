...
export function Prescribe() {
  ...
  const handleMedicineChange = (
    index: number,
    field: "name" | "dose" | "frequency" | "duration",
    value: string,
  ) => {
  ...
  if (isSuccess) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-blue-600 px-6 text-center">
        ...
        <p className="text-blue-100 font-medium mb-12">
          The digital prescription has been sent to the patient's app securely.
        </p>

        <div className="w-full space-y-4">
          <Button
            onClick={() => router.history.back()}
            className="w-full h-14 rounded-2xl bg-white text-blue-600 font-bold text-[15px] hover:bg-slate-50"
          >
            Back to Patient
          </Button>
          <Button
            onClick={() => router.navigate({ to: "/doctor" })}
            variant="outline"
            className="w-full h-14 rounded-2xl bg-blue-700/50 border-none text-white font-bold text-[15px] hover:bg-blue-700"
          >
            Go to Dashboard
          </Button>
        </div>
    ...
    <div className="flex flex-col min-h-screen bg-[#F8FAFC]">
        ...
        <div>
          ...
          <div className="space-y-4">
            {medicines.map((med, index) => (
              <div
                key={index}
                className="bg-white p-4 rounded-3xl border border-slate-100 shadow-sm relative"
              >
                {medicines.length > 1 && (
                  <button
                    onClick={() => removeMedicine(index)}
                    className="absolute -top-2 -right-2 h-6 w-6 bg-red-100 text-red-600 rounded-full flex items-center justify-center"
                  >
                    <X className="h-3 w-3" />
                  </button>
                )}
      ...
      <div className="fixed bottom-0 left-0 right-0 p-4 bg-white border-t border-slate-100 pb-safe z-50 shadow-[0_-10px_20px_-10px_rgba(0,0,0,0.05)]">
        <Button
          ...
          className="w-full h-14 rounded-2xl bg-blue-600 text-white font-bold text-[15px] shadow-lg shadow-blue-600/30 hover:bg-blue-700"
        >
