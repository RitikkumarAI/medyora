        ...
        <div className="max-w-md w-full p-8 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 shadow-xl text-center space-y-6">
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed font-medium">
              This area is restricted to verified medical specialists and clinic staff for patient
              queue telemetry and prescription authoring.
            </p>
          </div>
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] dark:bg-slate-950 text-slate-900 dark:text-slate-100 transition-colors">
      ...
      <div className="hidden md:flex min-h-screen">
        {/* Left Sidebar */}
        <aside
          className="w-64 bg-white dark:bg-slate-900 border-r border-slate-200/80 dark:border-slate-800 p-5 flex flex-col justify-between shrink-0 sticky top-0 h-screen overflow-y-auto"
          role="navigation"
          aria-label="Doctor Practice Navigation"
        >
          <div className="space-y-4">
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-1">
                  <p className="text-xs font-bold text-slate-900 dark:text-white truncate">
                    {user?.name || "Dr. Medical Specialist"}
                  </p>
            ...
            <nav className="space-y-1">
              {DOCTOR_SIDEBAR_NAV.map((item) => {
                const isActive =
                  currentPath === item.to ||
                  (item.to !== "/doctor" && currentPath.startsWith(item.to));
                return (
                  <Link
        ...
        <main
          id="main-content"
          tabIndex={-1}
          className="flex-1 min-w-0 overflow-x-hidden p-6 lg:p-8 focus:outline-none"
        >
          <Outlet />
        </main>
      </div>
      ...
      <div className="flex md:hidden flex-col min-h-screen">
        ...
        <main
          id="main-content"
          tabIndex={-1}
          className="flex-1 overflow-y-auto pb-24 focus:outline-none"
        >
          <Outlet />
        </main>
        ...
        <nav
          className="fixed bottom-0 left-0 right-0 bg-white/95 dark:bg-slate-900/95 backdrop-blur border-t border-slate-100 dark:border-slate-800 p-2 pb-safe z-50 shadow-[0_-10px_20px_-10px_rgba(0,0,0,0.05)]"
          role="navigation"
        ...
        >
          <div className="flex items-center justify-around">
            <Link
              to="/doctor"
              className={`flex flex-col items-center gap-1.5 px-4 py-2 rounded-2xl transition-all ${currentPath === "/doctor" ? "text-blue-600 dark:text-blue-400 bg-blue-50/80 dark:bg-blue-950/50 scale-105" : "text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300"}`}
            >
              <LayoutDashboard
                className={`h-6 w-6 ${currentPath === "/doctor" ? "fill-blue-100 dark:fill-blue-900" : ""}`}
              />
            ...
            <Link
              to="/doctor/calendar"
              className={`flex flex-col items-center gap-1.5 px-4 py-2 rounded-2xl transition-all ${currentPath.includes("/doctor/calendar") ? "text-blue-600 dark:text-blue-400 bg-blue-50/80 dark:bg-blue-950/50 scale-105" : "text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300"}`}
            >
              <Calendar
                className={`h-6 w-6 ${currentPath.includes("/doctor/calendar") ? "fill-blue-100 dark:fill-blue-900" : ""}`}
              />
            ...
            <Link
              to="/doctor/patients"
              className={`flex flex-col items-center gap-1.5 px-4 py-2 rounded-2xl transition-all ${currentPath.includes("/doctor/patients") ? "text-blue-600 dark:text-blue-400 bg-blue-50/80 dark:bg-blue-950/50 scale-105" : "text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300"}`}
            >
              <Users
                className={`h-6 w-6 ${currentPath.includes("/doctor/patients") ? "fill-blue-100 dark:fill-blue-900" : ""}`}
              />
            ...
            <Link
              to="/doctor/settings"
              className={`flex flex-col items-center gap-1.5 px-4 py-2 rounded-2xl transition-all ${currentPath.includes("/doctor/settings") ? "text-blue-600 dark:text-blue-400 bg-blue-50/80 dark:bg-blue-950/50 scale-105" : "text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300"}`}
            >
              <Settings
                className={`h-6 w-6 ${currentPath.includes("/doctor/settings") ? "fill-blue-100 dark:fill-blue-900" : ""}`}
              />
