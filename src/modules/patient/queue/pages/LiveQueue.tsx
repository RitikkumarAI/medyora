...
import { 
  ArrowLeft, Users, Clock, AlertCircle, CheckCircle2, MapPin, 
  RefreshCcw, Bell, Repeat, X, Sparkles 
} from "lucide-react";
...
export function LiveQueue() {
  ...
  const handleConfirmReschedule = () => {
    ...
    toast.success(`Rescheduled to today at ${selectedRescheduleSlot}. Your new Token is #${newToken}`);
    ...
    <div className="flex flex-col min-h-screen bg-[#F8FAFC] dark:bg-slate-950 text-slate-900 dark:text-slate-100 font-sans transition-colors pb-24">
      ...
      <header className="sticky top-0 z-40 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md px-4 pt-6 pb-4 shadow-xs border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button variant="ghost" size="icon" className="h-9 w-9 shrink-0 bg-slate-50 dark:bg-slate-800 rounded-full border border-slate-100 dark:border-slate-700" onClick={() => window.location.reload()}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-start">
          ...
          <div className="md:col-span-6 space-y-6">
            ...
            <div className="bg-white dark:bg-slate-900 rounded-3xl p-5 border border-slate-100 dark:border-slate-800 shadow-xs flex items-center justify-between">
              <div className="flex items-center gap-3">
                <img src={doctor.image} alt={doctor.clinic.name} className="h-12 w-12 rounded-2xl object-cover border border-slate-100 dark:border-slate-800" />
                <div>
                  <h3 className="font-bold text-sm text-slate-900 dark:text-white leading-tight">{doctor.clinic.name}</h3>
                  <p className="text-xs text-slate-500 dark:text-slate-400 flex items-center gap-1 mt-0.5">
                    <MapPin className="h-3 w-3" /> {doctor.clinic.address}, {doctor.clinic.city}
                  </p>
                </div>
              </div>
              <Button asChild size="sm" variant="outline" className="rounded-xl text-xs font-bold border-slate-200 dark:border-slate-700">
              ...
              <div className="relative z-10 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3 pt-2">
                  <div className="bg-white/10 backdrop-blur-xs rounded-2xl p-3 border border-white/15">
                    ...
                    <p className="text-[11px] font-medium text-blue-100 uppercase tracking-wider">Patients Ahead</p>
                  </div>
                  <div className="bg-white/10 backdrop-blur-xs rounded-2xl p-3 border border-white/15">
                    <p className="text-2xl font-black">{estWaitTime} <span className="text-xs font-normal">MIN</span></p>
                    <p className="text-[11px] font-medium text-blue-100 uppercase tracking-wider">Est. Wait Time</p>
                  </div>
                </div>
            ...
            <div className="bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800 rounded-3xl p-4 flex items-center justify-between gap-3">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h4 className="text-xs font-bold text-amber-900 dark:text-amber-200">Running late for this appointment?</h4>
                  <p className="text-[11px] text-amber-700 dark:text-amber-400">Reschedule to a later slot today without paying again.</p>
                </div>
              </div>
              <Button 
                onClick={() => setShowRescheduleModal(true)} 
          ...
          <div className="md:col-span-6 bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-100 dark:border-slate-800 shadow-xs space-y-4">
            ...
            <div className="space-y-2.5">
              {[
                ...
                { token: currentToken, status: "Consulting", isCurrent: true, isUser: currentToken === myToken },
                { token: myToken, status: "Your Turn", isCurrent: false, isUser: true },
              ].filter(item => item.token > 0).map((item) => (
                <div 
                  key={item.token}
                ...
                >
                  <div className="flex items-center gap-3">
                    <span className={`h-8 w-8 rounded-xl flex items-center justify-center font-black text-xs ${
                      item.isUser ? "bg-blue-600 text-white" : item.isCurrent ? "bg-amber-500 text-white" : "bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-300"
                    }`}>
                      #{item.token}
                    </span>
                    <div>
                      ...
                      <p className="text-[10px] text-slate-500 dark:text-slate-400">{doctor.fullName}</p>
                    </div>
                  </div>
            ...
            <div className="pt-4 border-t border-slate-100 dark:border-slate-800 space-y-2">
              <h4 className="font-bold text-xs text-slate-900 dark:text-white">Clinic Arrival Instructions</h4>
              <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
                Please arrive at {doctor.clinic.name} 15 minutes before your estimated turn time. Token calls are announced over the clinic audio system.
              </p>
            </div>
          </div>
          ...
          <div className="relative w-full max-w-md bg-white dark:bg-slate-900 rounded-t-[36px] sm:rounded-3xl p-6 shadow-2xl space-y-4 animate-in slide-in-from-bottom-full duration-300 z-10 border border-slate-200 dark:border-slate-800">
            <div className="flex items-center justify-between pb-3 border-b border-slate-100 dark:border-slate-800">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="font-bold text-base text-slate-900 dark:text-white">Running Late?</h3>
                  <p className="text-xs text-slate-500 dark:text-slate-400 font-medium">Pick a later token today without losing queue priority</p>
                </div>
              </div>
              <button onClick={() => setShowRescheduleModal(false)} className="h-8 w-8 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-slate-500">
                <X className="h-4 w-4" />
              </button>
            </div>

            <div className="bg-slate-50 dark:bg-slate-800/60 p-3 rounded-2xl text-xs space-y-1">
              <p className="font-bold text-slate-800 dark:text-slate-200">{doctor.fullName} • {doctor.clinic.name}</p>
