...
import { 
  ArrowLeft, Search, Clock, MapPin, Ticket, MessageSquare, 
  Repeat, User, AlertCircle, X, Bell, CheckCircle2, ShieldCheck, Sparkles,
  FileText, CalendarPlus, Download, Stethoscope, Pill, FlaskConical, ArrowRight, Share2
} from "lucide-react";
    ...
    <div className="flex flex-col min-h-screen bg-[#F8FAFC] dark:bg-slate-950 text-slate-900 dark:text-slate-100 pb-24 font-sans transition-colors">
      ...
      <header className="sticky top-0 z-40 bg-white dark:bg-slate-900 px-4 pt-6 pb-3 shadow-xs border-b border-slate-100 dark:border-slate-800">
        <div className="flex items-center gap-3 mb-3">
          ...
          <div>
            <h1 className="text-xl font-black text-slate-900 dark:text-white leading-tight">My Appointments</h1>
            <p className="text-xs text-slate-500 dark:text-slate-400 font-medium">{appointments.length} Total Bookings</p>
          </div>
        </div>
        ...
        <div className="p-3.5 rounded-2xl bg-gradient-to-r from-blue-600 to-indigo-600 text-white shadow-md flex items-center justify-between gap-3">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[11px] text-blue-100">Dr. Rajesh Sharma recommended a 15-day review consultation</p>
            </div>
          </div>
      ...
      <main className="flex-1 overflow-y-auto px-4 py-4 space-y-4 max-w-3xl mx-auto w-full">
        {list.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <h3 className="text-base font-bold text-slate-900 dark:text-white">No {tab.toLowerCase()} appointments</h3>
            <p className="text-xs text-slate-500 mt-1 max-w-[240px]">
              {tab === "Upcoming" ? "You don't have any active appointments scheduled." : `No appointments in ${tab.toLowerCase()} list.`}
            </p>
            {tab === "Upcoming" && (
              <Button asChild className="mt-4 rounded-2xl bg-blue-600 font-bold text-xs h-10 px-5 shadow-md shadow-blue-600/20">
          ...
          list.map((a) => (
            <div key={a.id} className="bg-white dark:bg-slate-900 rounded-3xl p-4.5 border border-slate-200/80 dark:border-slate-800 shadow-xs space-y-3.5">
              ...
              <div className="flex gap-3.5 items-start">
                <img src={a.image} alt={a.doctorName} className="h-14 w-14 rounded-2xl object-cover border border-slate-100 shrink-0" />
                <div className="flex-1 min-w-0">
                  <h4 className="font-bold text-[15px] text-slate-900 dark:text-white truncate">{a.doctorName}</h4>
                  <p className="text-xs font-semibold text-blue-600 dark:text-blue-400">{a.speciality}</p>
                  <p className="text-[11px] text-slate-500 dark:text-slate-400 truncate mt-0.5">{a.clinic} {a.city ? `• ${a.city}` : ""}</p>
                  ...
                  <div className="flex flex-wrap items-center gap-1.5 mt-2">
                    <span className="inline-flex items-center gap-1 text-[11px] font-bold text-slate-700 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded-md">
                      ...
                      For: {a.patient || "Self"} {a.patientDetails?.relation ? `(${a.patientDetails.relation})` : ""}
                    </span>
                    {a.paymentMode && (
                      <span className={`text-[10px] font-bold px-2 py-0.5 rounded-md ${a.paymentMode === 'online' ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300' : 'bg-amber-50 text-amber-800 dark:bg-amber-950 dark:text-amber-300'}`}>
              ...
              <div className="pt-1 flex flex-wrap gap-2">
                {a.status === "Confirmed" && (
                  <>
                    <Button asChild variant="outline" size="sm" className="flex-1 h-9 rounded-xl text-xs font-bold border-blue-200 text-blue-700 bg-blue-50/50 hover:bg-blue-100">
                ...
                {a.status === "Cancelled" && (
                  <Button asChild variant="default" size="sm" className="w-full h-9 rounded-xl text-xs font-bold bg-blue-600">
                    <Link to="/booking/$doctorId" params={{ doctorId: a.doctorId }}>Rebook Appointment</Link>
                  </Button>
                )}
              </div>
            </div>
            ...
            >
              ...
              <div className="p-5 bg-gradient-to-r from-blue-600 to-indigo-700 text-white flex items-center justify-between shrink-0">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-blue-100">Token: {opdModalTarget.token} • Ref: {opdModalTarget.reference}</p>
                  </div>
                </div>
                <button onClick={() => setOpdModalTarget(null)} className="text-white/80 hover:text-white">
                  <X className="h-5 w-5" />
                </button>
              </div>
              ...
              <div className="p-6 overflow-y-auto space-y-4 text-xs">
                ...
                <div className="flex items-center justify-between pb-3 border-b border-slate-100 dark:border-slate-800">
                  <div>
                    <h4 className="font-black text-sm text-slate-900 dark:text-white">{opdModalTarget.doctorName}</h4>
                    <p className="text-blue-600 dark:text-blue-400 font-semibold">{opdModalTarget.speciality}</p>
                ...
                <div className="grid grid-cols-3 gap-2 bg-slate-50 dark:bg-slate-800/60 p-3 rounded-2xl border border-slate-100 dark:border-slate-700">
                  <div>
                    ...
                    <strong className="text-slate-900 dark:text-white">{opdModalTarget.patient || "Ritik Kumar"}</strong>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <strong className="text-slate-900 dark:text-white">{opdModalTarget.date}</strong>
                  </div>
                </div>
                ...
                <div className="space-y-1.5 p-3.5 rounded-2xl bg-blue-50/60 dark:bg-blue-950/40 border border-blue-100 dark:border-blue-900">
                  ...
                  <p className="text-slate-700 dark:text-slate-300 font-medium">
                    Essential Hypertension (Stage 1) with Mild Exertional Fatigue. Cardiac rhythm regular.
                  </p>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="space-y-1.5">
                    {[
                      { name: "Tab. Telmisartan 40mg", dosage: "1 Tab - Morning (After Breakfast)", days: "30 Days" },
                      { name: "Tab. Rosuvastatin 10mg", dosage: "1 Tab - Night (Bedtime)", days: "30 Days" },
                      { name: "Cap. Vitamin D3 60K IU", dosage: "1 Cap - Weekly Once (Sunday Morning)", days: "8 Weeks" },
                    ].map((med, i) => (
                      <div key={i} className="p-2.5 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700 flex justify-between items-center">
                        <div>
              ...
              <div className="p-4 bg-slate-50 dark:bg-slate-850 border-t border-slate-100 dark:border-slate-800 flex gap-2 shrink-0">
                ...
                >
                  <Link to="/booking/$doctorId" params={{ doctorId: opdModalTarget.doctorId }} onClick={() => setOpdModalTarget(null)}>
                    Book Follow-up Now
                  </Link>
                </Button>
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
          ...
          <div className="relative w-full max-w-md bg-white rounded-t-[36px] sm:rounded-3xl p-6 shadow-2xl space-y-4 animate-in slide-in-from-bottom-full duration-300 z-10">
            <div className="flex items-center justify-between pb-3 border-b border-slate-100">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 font-medium">Running late? Pick a later slot today</p>
                </div>
              </div>
              <button onClick={() => setRescheduleTarget(null)} className="h-8 w-8 rounded-full bg-slate-100 flex items-center justify-center text-slate-500">
                <X className="h-4 w-4" />
              </button>
            </div>

            <div className="bg-slate-50 p-3 rounded-2xl text-xs space-y-1">
              <p className="font-bold text-slate-800">{rescheduleTarget.doctorName} • {rescheduleTarget.clinic}</p>
              <p className="text-slate-500">Current Slot: {rescheduleTarget.time} (Token {rescheduleTarget.token})</p>
            </div>
            ...
            <div className="bg-blue-50 border border-blue-100 p-3 rounded-2xl text-xs text-blue-900 flex items-start gap-2">
              ...
              <span>A new token number will be generated immediately and sent to your WhatsApp.</span>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-[#ECE5DD] w-full max-w-sm rounded-[32px] overflow-hidden shadow-2xl animate-in zoom-in-95">
            <div className="bg-[#075E54] text-white p-4 flex items-center justify-between">
              ...
              <button onClick={() => setWhatsAppModalTarget(null)} className="text-white hover:text-emerald-200">
                <X className="h-5 w-5" />
              </button>
            </div>
            ...
            <div className="p-4 space-y-3 max-h-[380px] overflow-y-auto">
              <div className="bg-white p-3.5 rounded-2xl rounded-tl-sm shadow-sm text-xs text-slate-800 space-y-2 border border-slate-200">
                ...
                <p>Hello <strong>{whatsAppModalTarget.patient || "Rahul"}</strong>,</p>
                <p>Your appointment reminder with <strong>{whatsAppModalTarget.doctorName}</strong>:</p>
                <div className="bg-slate-50 p-2.5 rounded-xl space-y-1 font-medium text-[11px]">
                  <p>📍 <strong>Clinic:</strong> {whatsAppModalTarget.clinic}</p>
                  <p>🗓️ <strong>Scheduled For:</strong> {whatsAppModalTarget.date} at {whatsAppModalTarget.time}</p>
                  <p>🎟️ <strong>Current Token:</strong> {whatsAppModalTarget.token}</p>
                </div>
                ...
                <p className="text-[10px] text-slate-400 text-right">Sent via WhatsApp • Delivered</p>
              </div>
              ...
              <div className="bg-white p-3 rounded-2xl rounded-tl-sm shadow-sm text-xs text-slate-800 space-y-1 border border-slate-200">
                ...
                <p className="text-[11px]">Queue is moving on schedule. 2 patients currently ahead of you.</p>
                <p className="text-[10px] text-slate-400 text-right">Active Sync</p>
              </div>
            </div>

            <div className="p-3 bg-white border-t border-slate-200">
              <Button onClick={() => setWhatsAppModalTarget(null)} className="w-full rounded-xl bg-[#075E54] text-white font-bold text-xs h-10">
                Close
              </Button>
            </div>
          </div>
