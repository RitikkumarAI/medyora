...
import { 
  ArrowLeft, Calendar, FileText, Pill, Heart, Users, 
  Bell, Settings, HelpCircle, LogOut, Moon, Sun, ChevronRight,
  CreditCard
} from "lucide-react";
...
export function PatientDashboard() {
  ...
  const gridItems = [
    { icon: Calendar, label: "Appointments", count: "3", link: "/patient/appointments", color: "bg-blue-100 text-blue-600 dark:bg-blue-900/30 dark:text-blue-400" },
    { icon: FileText, label: "Records", count: "12", link: "/patient/appointments", color: "bg-purple-100 text-purple-600 dark:bg-purple-900/30 dark:text-purple-400" },
    { icon: Pill, label: "Prescriptions", count: "5", link: "/patient/prescriptions", color: "bg-emerald-100 text-emerald-600 dark:bg-emerald-900/30 dark:text-emerald-400" },
    { icon: Heart, label: "Favorites", count: "8", link: "/patient/favorites", color: "bg-rose-100 text-rose-600 dark:bg-rose-900/30 dark:text-rose-400" },
    { icon: Users, label: "Family", count: "4", link: "/patient/family", color: "bg-amber-100 text-amber-600 dark:bg-amber-900/30 dark:text-amber-400" },
    { icon: Bell, label: "Notifications", count: "2", link: "/patient/notifications", color: "bg-indigo-100 text-indigo-600 dark:bg-indigo-900/30 dark:text-indigo-400" },
  ];

  const listItems = [
    { icon: CreditCard, label: "Payment History", link: "/patient/payment/history", destructive: false },
  ...
  return (
    <div className={`flex flex-col min-h-screen transition-colors ${isDarkMode ? 'bg-slate-950' : 'bg-[#F8FAFC]'}`}>
      ...
      <div className="bg-white dark:bg-slate-900 pb-6 rounded-b-[40px] shadow-sm border-b border-slate-100 dark:border-slate-800 transition-colors">
        <header className="px-4 pt-6 pb-2 flex items-center justify-between">
          ...
          <Button variant="ghost" size="icon" onClick={toggleDarkMode} className="h-10 w-10 shrink-0 bg-slate-50 dark:bg-slate-800 rounded-full border border-slate-100 dark:border-slate-700">
        ...
        <div className="px-6 flex items-center gap-5 mt-2">
          ...
          <div>
            ...
            <p className="text-sm font-medium text-slate-500 dark:text-slate-400 mt-0.5">+91 98765 43210</p>
            <div className="flex items-center gap-2 mt-2">
              <span className="bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider">Premium</span>
            </div>
          </div>
        </div>
      </div>
            ...
            {listItems.map((item, index) => (
              ...
              >
                <div className="flex items-center gap-4">
                  <div className={`h-10 w-10 rounded-2xl flex items-center justify-center ${item.destructive ? 'bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400'}`}>
                    <item.icon className="h-5 w-5" />
                  </div>
                  <span className={`text-sm font-bold ${item.destructive ? 'text-red-600 dark:text-red-400' : 'text-slate-700 dark:text-slate-300'}`}>{item.label}</span>
                </div>
                {!item.destructive && <ChevronRight className="h-5 w-5 text-slate-300 dark:text-slate-600" />}
              </Link>
            ))}
          </div>
        </section>
