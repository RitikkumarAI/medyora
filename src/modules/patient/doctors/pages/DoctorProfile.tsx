...
import { 
  ArrowLeft, Share2, Heart, Star, MapPin, CheckCircle2, 
  ChevronDown, PhoneCall, GraduationCap, Languages, 
  Image as ImageIcon, Award, Clock, Sparkles, Building2, Stethoscope,
  Play, Video, Car, Accessibility, Navigation, ShieldCheck, Crown, Flame, X
} from "lucide-react";
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#F8FAFC] dark:bg-slate-950 text-slate-900 dark:text-slate-100 font-sans transition-colors">
      ...
      <div className="lg:hidden absolute top-0 left-0 right-0 z-30 flex items-center justify-between p-4 bg-gradient-to-b from-black/60 to-transparent">
        <Button variant="ghost" size="icon" onClick={() => router.history.back()} className="h-10 w-10 bg-white/20 backdrop-blur-md rounded-full border border-white/30 text-white">
          <ArrowLeft className="h-5 w-5" />
        </Button>
        <div className="flex gap-2">
          <Button variant="ghost" size="icon" onClick={() => toast.success("Doctor profile link copied!")} className="h-10 w-10 bg-white/20 backdrop-blur-md rounded-full border border-white/30 text-white">
            <Share2 className="h-5 w-5" />
          </Button>
          <Button variant="ghost" size="icon" onClick={() => setIsFavorite(!isFavorite)} className="h-10 w-10 bg-white/20 backdrop-blur-md rounded-full border border-white/30">
      ...
      <div className="hidden lg:block max-w-6xl mx-auto w-full px-6 py-8">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-2">
            <Button variant="outline" size="sm" onClick={() => toast.success("Doctor profile link copied!")} className="rounded-xl text-xs font-bold gap-1.5 border-slate-200 dark:border-slate-800">
              <Share2 className="h-3.5 w-3.5" /> Share
            </Button>
            <Button variant="outline" size="sm" onClick={() => setIsFavorite(!isFavorite)} className="rounded-xl text-xs font-bold gap-1.5 border-slate-200 dark:border-slate-800">
        ...
        <div className="grid grid-cols-12 gap-8 items-start">
          ...
          <div className="col-span-4 sticky top-24 bg-white dark:bg-slate-900 rounded-[32px] p-6 border border-slate-200/80 dark:border-slate-800 shadow-xs text-center space-y-5">
            ...
            <div className="relative mx-auto h-44 w-44 rounded-3xl overflow-hidden shadow-md border-2 border-white dark:border-slate-800 group">
              <img src={doctor.image} alt={doctor.fullName} className="h-full w-full object-cover object-top group-hover:scale-105 transition-transform duration-300" />
            ...
            <div>
              ...
              <p className="text-xs font-bold text-blue-600 dark:text-blue-400 mt-0.5">{doctor.speciality}</p>
            ...
            <div className="p-2.5 rounded-2xl bg-blue-50 dark:bg-blue-950/60 border border-blue-100 dark:border-blue-900/60 text-[11px] text-blue-900 dark:text-blue-200 text-left flex items-start gap-2">
              ...
              <div>
                <strong className="block font-black text-blue-800 dark:text-blue-300">Live OPD Prediction:</strong>
                Dr. {doctor.fullName.split(" ")[1] || "Sharma"} usually gets free after 05:30 PM (Avg wait: ~12 mins).
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-2 bg-slate-50 dark:bg-slate-800/80 rounded-2xl p-3 border border-slate-100 dark:border-slate-700">
              <div>
                <p className="text-base font-black text-slate-900 dark:text-white">{doctor.experience}+</p>
              ...
              <div>
                <p className="text-base font-black text-slate-900 dark:text-white">{doctor.totalReviews}+</p>
            ...
            <div className="pt-3 border-t border-slate-100 dark:border-slate-800 space-y-3">
              ...
              <Button asChild className="w-full h-12 rounded-2xl bg-blue-600 hover:bg-blue-700 text-white font-black text-xs shadow-lg shadow-blue-600/25">
                <Link to="/patient/doctor/$doctorId/book" params={{ doctorId: doctor.id }}>
                  Book In-Clinic / Video Consultation
                </Link>
              </Button>

              <Button asChild variant="outline" className="w-full h-11 rounded-2xl border-slate-200 dark:border-slate-700 font-bold text-xs text-slate-700 dark:text-slate-200 hover:bg-slate-50">
                <a href={`tel:${doctor.phone || "+919876543210"}`} className="flex items-center justify-center gap-2">
          ...
          <div className="col-span-8 space-y-6">
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl p-1.5 border border-slate-200/80 dark:border-slate-800 shadow-xs flex gap-1">
              {(["about", "clinics", "experience", "reviews", "gallery"] as TabOption[]).map(tab => (
                <button
                  key={tab}
            ...
            {activeTab === "about" && (
              <div className="space-y-6">
                <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-200/80 dark:border-slate-800 shadow-xs space-y-4">
                  <h3 className="font-black text-base text-slate-900 dark:text-white">About Doctor & Treatment Philosophy</h3>
                  ...
                  {doctor.diseases && doctor.diseases.length > 0 && (
                    <div className="pt-4 border-t border-slate-100 dark:border-slate-800">
                      <h4 className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-2">Specialties & Conditions Treated</h4>
                      <div className="flex flex-wrap gap-1.5">
                        {doctor.diseases.map((d, i) => (
                          <span key={i} className="bg-blue-50 dark:bg-blue-950/60 text-blue-700 dark:text-blue-300 text-xs font-bold px-3 py-1 rounded-full border border-blue-100 dark:border-blue-900">
                            {d}
                          </span>
                        ))}
                      </div>
                ...
                <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-200/80 dark:border-slate-800 shadow-xs space-y-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                    <div className="p-3.5 rounded-2xl bg-slate-50 dark:bg-slate-800/60 border border-slate-100 dark:border-slate-700 flex items-center gap-3">
                      ...
                      <div>
                        <strong className="block text-slate-900 dark:text-white">Parking Facility:</strong>
                        <span className="text-slate-500 dark:text-slate-400">Dedicated Free Car & Bike Parking</span>
                      </div>
                    </div>
                    ...
                    <div className="p-3.5 rounded-2xl bg-slate-50 dark:bg-slate-800/60 border border-slate-100 dark:border-slate-700 flex items-center gap-3">
                      ...
                      <div>
                        <strong className="block text-slate-900 dark:text-white">Wheelchair & Lift:</strong>
                        <span className="text-slate-500 dark:text-slate-400">Ramp Access & Stretcher Elevator</span>
                      </div>
                    </div>
                    ...
                    <div className="p-3.5 rounded-2xl bg-slate-50 dark:bg-slate-800/60 border border-slate-100 dark:border-slate-700 flex items-center gap-3">
                      ...
                      <div>
                        <strong className="block text-slate-900 dark:text-white">Floor & Wing:</strong>
                        <span className="text-slate-500 dark:text-slate-400">2nd Floor, Suite 204 (OPD Wing)</span>
                      </div>
                    </div>
                    ...
                    <div className="p-3.5 rounded-2xl bg-slate-50 dark:bg-slate-800/60 border border-slate-100 dark:border-slate-700 flex items-center gap-3">
                      ...
                      <div>
                        <strong className="block text-slate-900 dark:text-white">Prominent Landmark:</strong>
                        <span className="text-slate-500 dark:text-slate-400">Opposite Metro Pillar 104, Indiranagar</span>
                      </div>
                    </div>
                  </div>

                  <Button asChild variant="outline" className="w-full h-11 rounded-2xl border-blue-200 text-blue-700 font-bold text-xs hover:bg-blue-50">
                    <a href={`https://maps.google.com/?q=${encodeURIComponent(`${doctor.clinic.name} ${doctor.clinic.address}`)}`} target="_blank" rel="noreferrer" className="flex items-center justify-center gap-2">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-white dark:bg-slate-900 p-5 rounded-3xl border border-slate-200/80 dark:border-slate-800 shadow-xs flex items-center gap-3.5">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-bold text-slate-900 dark:text-white">{doctor.experience}+ Years Practice</p>
                    </div>
                  </div>
                  ...
                  <div className="bg-white dark:bg-slate-900 p-5 rounded-3xl border border-slate-200/80 dark:border-slate-800 shadow-xs flex items-center gap-3.5">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-bold text-slate-900 dark:text-white">{doctor.languages.join(", ")}</p>
                    </div>
                  </div>
                </div>
            ...
            {activeTab === "clinics" && (
              <div className="space-y-4">
                <div className="p-4 bg-blue-50 dark:bg-blue-950/50 rounded-2xl border border-blue-100 dark:border-blue-900 text-xs text-blue-900 dark:text-blue-200">
                  💡 <strong>Multi-Clinic OPD Schedule:</strong> {doctor.fullName} consults at multiple branches. Choose the clinic nearest to you.
                </div>

                {clinicsList.map((cln, i) => (
                  <div key={cln.id} className="bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-200/80 dark:border-slate-800 shadow-xs space-y-3">
                    <div className="flex items-start justify-between">
                      <div>
                        <span className="text-[10px] font-black uppercase px-2 py-0.5 rounded-md bg-blue-100 text-blue-700">
                          Branch #{i + 1}
                        </span>
                        <h4 className="font-bold text-base text-slate-900 dark:text-white mt-1">{cln.name}</h4>
                        <p className="text-xs text-slate-500 dark:text-slate-400">{cln.address}, {cln.area}, {cln.city}</p>
                      </div>
                      <span className="text-sm font-black text-slate-900 dark:text-white">₹{cln.fee}</span>
                    </div>
                    ...
                    <div className="pt-2 flex gap-2">
                      <Button asChild size="sm" className="flex-1 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-bold text-xs h-10">
                        <Link to="/patient/doctor/$doctorId/book" params={{ doctorId: doctor.id }}>
                          Book at this Branch
                        </Link>
                      </Button>
                      <Button asChild variant="outline" size="sm" className="rounded-xl border-slate-200 dark:border-slate-700 text-xs font-bold h-10">
            ...
            {activeTab === "experience" && (
              <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-200/80 dark:border-slate-800 shadow-xs space-y-4">
                <h3 className="font-bold text-base text-slate-900 dark:text-white">Medical Background & Education</h3>
                <div className="space-y-4">
                  <div className="flex gap-3">
                    ...
                    <div>
                      <h4 className="font-bold text-xs text-slate-900 dark:text-white">{doctor.qualification}</h4>
                      <p className="text-xs text-slate-500 mt-0.5">Premier Medical Institute, Board Certified</p>
                    </div>
                  </div>
                  ...
                  <div className="flex gap-3">
                    ...
                    <div>
                      <h4 className="font-bold text-xs text-slate-900 dark:text-white">{doctor.experience}+ Years Active Practice</h4>
                      <p className="text-xs text-slate-500 mt-0.5">Specialized in OPD consultations and inpatient medical supervision</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            {activeTab === "reviews" && (
              <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-200/80 dark:border-slate-800 shadow-xs space-y-4">
                <div className="flex items-center justify-between">
                  <h3 className="font-bold text-base text-slate-900 dark:text-white">Verified Patient Reviews ({REVIEWS.length})</h3>
                ...
                <div className="space-y-3">
                  {REVIEWS.map((review) => (
                    <div key={review.id} className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-800/60 border border-slate-100 dark:border-slate-700 space-y-2">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-1.5">
                          <span className="font-bold text-xs text-slate-900 dark:text-white">{review.patientName}</span>
                      ...
                      <span className="text-[10px] text-slate-400 block">{review.date} • Verified Clinic Visit</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {activeTab === "gallery" && (
              <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-200/80 dark:border-slate-800 shadow-xs space-y-4">
                <h3 className="font-bold text-base text-slate-900 dark:text-white">Clinic & Facility Photos</h3>
                <div className="grid grid-cols-3 gap-3">
                  {doctor.gallery?.map((img, i) => (
                    <img key={i} src={img} alt="Clinic" className="h-36 w-full object-cover rounded-2xl border border-slate-100 dark:border-slate-800" />
                  ))}
                </div>
              </div>
            )}
      ...
      <main className="lg:hidden flex-1 pb-40">
        <div className="relative h-[320px] w-full bg-slate-900">
          <img src={doctor.image} alt={doctor.fullName} className="h-full w-full object-cover object-top" />
        ...
        <div className="px-5 -mt-20 relative z-10 space-y-4">
          <div className="bg-white dark:bg-slate-900 rounded-[32px] p-5 shadow-lg border border-slate-100 dark:border-slate-800 space-y-3">
            ...
            <div className="flex items-center gap-1.5">
              <h1 className="text-xl font-bold text-slate-900 dark:text-white">{doctor.fullName}</h1>
              {doctor.verified && <CheckCircle2 className="h-4 w-4 text-blue-600" />}
            </div>
            <p className="text-sm font-medium text-blue-600">{doctor.speciality} • {doctor.qualification}</p>
            ...
            <div className="flex justify-between items-center bg-slate-50 dark:bg-slate-800/80 rounded-2xl p-3 border border-slate-100 dark:border-slate-700">
              <div className="text-center px-2 border-r border-slate-200 dark:border-slate-700 flex-1">
                <p className="text-lg font-black text-slate-900 dark:text-white">{doctor.experience}+</p>
              ...
              <div className="text-center px-2 flex-1">
                <p className="text-lg font-black text-slate-900 dark:text-white">{doctor.totalReviews}+</p>
          ...
          <section className="bg-white dark:bg-slate-900 p-5 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-xs space-y-2">
            ...
            <p className="text-xs text-slate-600 dark:text-slate-300 leading-relaxed">{doctor.about}</p>
          </section>

          <section className="bg-white dark:bg-slate-900 p-5 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-xs space-y-2">
            <h3 className="font-bold text-sm text-slate-900 dark:text-white">Clinic Location & Amenities</h3>
            <p className="text-xs font-bold text-slate-900 dark:text-white">{doctor.clinic.name}</p>
            <p className="text-xs text-slate-500">{doctor.clinic.address}, {doctor.clinic.city}</p>
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 p-4 bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl border-t border-slate-200 dark:border-slate-800 z-50 shadow-[0_-8px_30px_rgba(0,0,0,0.12)]">
        <div className="flex gap-3 max-w-lg mx-auto">
          <a href={`tel:${doctor.phone || "+919876543210"}`} className="flex-1 flex flex-col items-center justify-center h-[56px] rounded-2xl bg-blue-50 text-blue-600 font-bold text-[13px] border border-blue-100 hover:bg-blue-100 transition-colors">
            <PhoneCall className="h-4 w-4 mb-0.5" />
            Call Clinic
          </a>
          <Link to="/patient/doctor/$doctorId/book" params={{ doctorId: doctor.id }} className="flex-[2] flex items-center justify-between h-[56px] px-6 rounded-2xl bg-blue-600 text-white shadow-lg shadow-blue-600/30 hover:bg-blue-700 transition-all">
            <div>
            ...
            >
              <div className="p-4 bg-slate-850 flex items-center justify-between border-b border-slate-800">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-bold text-xs text-white">{doctor.fullName} — 30s Doctor Intro</span>
                </div>
              ...
              <div className="relative aspect-video bg-black flex items-center justify-center">
                <img src={doctor.image} alt={doctor.fullName} className="h-full w-full object-cover opacity-60" />
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-6 space-y-3">
                  ...
                  <p className="text-xs text-white/90 max-w-xs font-medium">
                    "Hello, I am {doctor.fullName}. I specialize in evidence-based {doctor.speciality} care with transparent treatment protocols."
                  </p>
                </div>
              </div>

              <div className="p-4 bg-slate-850 flex items-center justify-between text-xs text-slate-300">
                <span>⭐ {doctor.rating} Rating • {doctor.experience}+ Years Exp</span>
                <Button asChild size="sm" className="rounded-xl bg-blue-600 text-white font-bold">
                  <Link to="/patient/doctor/$doctorId/book" params={{ doctorId: doctor.id }} onClick={() => setIsVideoModalOpen(false)}>
                    Book Consultation
                  </Link>
                </Button>
              </div>
