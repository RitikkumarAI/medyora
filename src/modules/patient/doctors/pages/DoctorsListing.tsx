...
import { 
  ArrowLeft, Search, SlidersHorizontal, Grid2X2, List as ListIcon, 
  Star, MapPin, Clock, Heart, ShieldCheck, ChevronDown, Check, X
} from "lucide-react";
...
export function DoctorsListing() {
  ...
  const [favorites, setFavorites] = useState<Set<string>>(new Set(DOCTORS.filter(d => d.isFavorite).map(d => d.id)));
  ...
  let displayDoctors = DOCTORS.filter(doc => {
    // Search
    if (searchQuery && !doc.fullName.toLowerCase().includes(searchQuery.toLowerCase()) && !doc.speciality.toLowerCase().includes(searchQuery.toLowerCase())) return false;
    // Filters
    ...
    <div className="flex flex-col min-h-screen bg-[#F8FAFC] dark:bg-slate-950 pb-24 transition-colors">
      ...
      <header className="sticky top-0 z-40 bg-white dark:bg-slate-900 px-4 pt-6 pb-3 shadow-sm border-b border-slate-100 dark:border-slate-800 transition-colors">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            <Button variant="ghost" size="icon" onClick={() => router.history.back()} className="h-10 w-10 shrink-0 bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700 shadow-sm rounded-full">
              <ArrowLeft className="h-5 w-5 text-slate-700 dark:text-slate-200" />
            </Button>
            <div>
              <h1 className="text-lg font-bold text-slate-900 dark:text-white">Find Doctors</h1>
              <p className="text-xs font-medium text-slate-500 dark:text-slate-400">{displayDoctors.length} Specialists</p>
            </div>
          </div>
          <div className="flex gap-2">
            <Button variant="ghost" size="icon" onClick={() => setViewMode(viewMode === "list" ? "grid" : "list")} className="h-10 w-10 rounded-full bg-slate-50 dark:bg-slate-800 text-slate-600 dark:text-slate-300">
              {viewMode === "list" ? <Grid2X2 className="h-4 w-4" /> : <ListIcon className="h-4 w-4" />}
            </Button>
          </div>
        </div>
        ...
        <div className="flex gap-2">
          ...
          <Button onClick={() => setIsFilterOpen(true)} variant="outline" size="icon" className="h-10 w-10 rounded-xl border-slate-100 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-sm text-blue-600 dark:text-blue-400 shrink-0 relative">
        ...
        <div className="flex gap-2 overflow-x-auto scrollbar-hide mt-3 pb-1">
          {(["relevance", "rating", "price_low", "experience"] as SortOption[]).map(sort => (
            ...
            >
              {sort === "relevance" ? "All" : sort === "rating" ? "Top Rated" : sort === "price_low" ? "Lowest Fee" : "Most Experienced"}
            </button>
          ))}
        </div>
      </header>
      ...
      <main className="flex-1 overflow-y-auto px-4 py-4 relative">
        {isLoading ? (
          ...
          <div className="mt-10">
            <EmptyState title="No Doctors Found" description="Try adjusting your filters or search query." />
          </div>
        ) : (
          <div className={viewMode === "grid" ? "grid grid-cols-2 gap-3" : "space-y-3"}>
            {displayDoctors.map(doc => (
              ...
              >
                ...
                >
                  <Heart className={`h-3.5 w-3.5 ${favorites.has(doc.id) ? 'fill-red-500 text-red-500' : 'text-slate-400'}`} />
                </button>
                ...
                <div className="flex flex-1 flex-col justify-center min-w-0">
                  <div className="flex items-center gap-1 mb-0.5">
                    <h4 className="font-bold text-[14px] text-slate-900 dark:text-white truncate">{doc.fullName}</h4>
                    {doc.verified && <ShieldCheck className="h-3 w-3 text-blue-600 dark:text-blue-400 shrink-0" />}
                  </div>
                  <p className="text-[11px] font-medium text-slate-500 dark:text-slate-400 truncate mb-1">{doc.speciality}</p>
                  
                  <div className="flex items-center gap-1 mb-2">
                    <Star className="h-3.5 w-3.5 fill-amber-400 text-amber-400" />
                    <span className="text-[10px] font-bold text-slate-700 dark:text-slate-200">{doc.rating}</span>
                    ...
                    <span className="text-[10px] font-medium text-slate-500 dark:text-slate-400">{doc.distance} km</span>
                  </div>

                  <div className="flex items-center justify-between mt-auto pt-2 border-t border-slate-50 dark:border-slate-800">
                    <div className="font-black text-blue-600 dark:text-blue-400 text-[13px]">₹{doc.fee}</div>
            ...
            <div className="py-6 text-center">
               ...
               <p className="text-xs font-medium text-slate-500 dark:text-slate-400 mt-2">Loading more...</p>
            </div>
          </div>
        )}
      </main>
      ...
      {compareList.size > 0 && (
        <div className="fixed bottom-20 left-1/2 -translate-x-1/2 z-50 w-[90%] max-w-sm animate-in slide-in-from-bottom-10">
          <div className="bg-slate-900 text-white rounded-2xl p-3 px-5 shadow-2xl flex items-center justify-between">
            ...
            <Link to="/patient/compare" search={{ doctors: Array.from(compareList) }} className="h-9 px-4 rounded-xl bg-blue-600 hover:bg-blue-700 text-xs font-bold flex items-center justify-center">
              View
            </Link>
          </div>
        </div>
      ...
      {isFilterOpen && (
        <div className="fixed inset-0 z-50 flex flex-col justify-end">
          <div className="absolute inset-0 bg-slate-900/60 backdrop-blur-sm animate-in fade-in" onClick={() => setIsFilterOpen(false)} />
          <div className="relative bg-white dark:bg-slate-900 rounded-t-[40px] p-6 flex flex-col max-h-[85vh] animate-in slide-in-from-bottom-full duration-300 shadow-2xl border-t border-slate-100 dark:border-slate-800">
            ...
            <div className="flex items-center justify-between mb-6 shrink-0">
              ...
              <button onClick={() => { setFilterFee(2000); setFilterVerified(false); setFilterGender(null); }} className="text-xs font-bold text-blue-600 dark:text-blue-400">Reset</button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto space-y-8 pb-6 pr-2">
              ...
              <section>
                <div className="flex items-center justify-between mb-3">
                  <h3 className="font-bold text-sm text-slate-900 dark:text-white">Max Consultation Fee</h3>
                  <span className="text-sm font-black text-blue-600 dark:text-blue-400">₹{filterFee}</span>
                </div>
                <input 
                  type="range" 
              ...
              {/* Gender Filter */}
              <section>
                <h3 className="font-bold text-sm text-slate-900 dark:text-white mb-3">Doctor Gender</h3>
              ...
              <section>
                <label className="flex items-center justify-between cursor-pointer">
                  <div>
                    <h3 className="font-bold text-sm text-slate-900 dark:text-white">Verified Doctors Only</h3>
                    <p className="text-xs font-medium text-slate-500 dark:text-slate-400">Medyora certified specialists</p>
                  </div>
                  <div className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${filterVerified ? 'bg-blue-600' : 'bg-slate-200 dark:bg-slate-700'}`} onClick={() => setFilterVerified(!filterVerified)}>
                    <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${filterVerified ? 'translate-x-6' : 'translate-x-1'}`} />
                  </div>
                </label>
              </section>
            </div>
            ...
            <div className="pt-4 mt-auto border-t border-slate-100 dark:border-slate-800 shrink-0">
              <Button onClick={() => setIsFilterOpen(false)} className="w-full h-14 rounded-2xl bg-blue-600 hover:bg-blue-700 text-white font-bold text-sm shadow-lg shadow-blue-600/20">
                Apply Filters
              </Button>
            </div>
          </div>
