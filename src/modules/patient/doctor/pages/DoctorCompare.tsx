...
export function DoctorCompare() {
  ...
  const search = useSearch({ strict: false }) as { doctors?: string[] };
      ...
      {selectedDoctors.length < 2 ? (
        <div className="flex flex-col items-center justify-center p-8 text-center flex-1">
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-2 mb-4">
            Please select at least 2 doctors to compare.
          </p>
        ...
        <main className="flex-1 overflow-x-auto p-4 pb-24">
          <div className="flex gap-4 min-w-max pb-4">
            ...
            <div className="sticky left-0 bg-[#F8FAFC] dark:bg-slate-950 z-10 w-28 shrink-0 flex flex-col gap-2 pt-28 shadow-[10px_0_15px_-10px_rgba(0,0,0,0.05)]">
              <div className="h-12 flex items-center px-2 text-xs font-bold text-slate-400 uppercase tracking-wide">
                Experience
              </div>
              <div className="h-12 flex items-center px-2 text-xs font-bold text-slate-400 uppercase tracking-wide">
                Fees
              ...
              <div className="h-16 flex items-center px-2 text-xs font-bold text-slate-400 uppercase tracking-wide">
                City
              </div>
              <div className="h-24 flex items-center px-2 text-xs font-bold text-slate-400 uppercase tracking-wide">
                Languages
              </div>
            </div>
              ...
              <div key={doc.id} className="w-40 shrink-0 flex flex-col gap-2 relative">
                ...
                <div className="bg-white dark:bg-slate-900 p-3 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-xs flex flex-col items-center text-center h-28 justify-center relative">
                  <img
                    src={doc.image}
                    alt={doc.fullName}
                    className="h-12 w-12 rounded-full object-cover mb-2 border border-slate-100 dark:border-slate-800"
                  />
                  <h3 className="text-[13px] font-bold text-slate-900 dark:text-white leading-tight line-clamp-1">
                    {doc.fullName}
                  </h3>
                  <p className="text-[10px] text-blue-600 dark:text-blue-400 font-medium line-clamp-1">
                    {doc.speciality}
                  </p>
                </div>
                ...
                <div className="h-12 flex items-center justify-center bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800">
                  {doc.homeVisit ? (
                    ...
                    <Minus className="h-5 w-5 text-slate-300 dark:text-slate-700" />
                  )}
                </div>

                <div className="h-12 flex items-center justify-center bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800">
                  {(doc.clinic.online ?? true) ? (
                    <Check className="h-5 w-5 text-green-500" />
                  ) : (
                    <Minus className="h-5 w-5 text-slate-300 dark:text-slate-700" />
                  )}
                ...
                <div className="h-16 flex flex-col items-center justify-center bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 px-2 text-center">
                  ...
                  <span className="text-[11px] font-bold text-slate-700 dark:text-slate-300 line-clamp-2">
                    {doc.city}
                  </span>
                </div>
                ...
                <div className="h-24 flex items-center justify-center bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 p-2">
                  <div className="flex flex-wrap justify-center gap-1">
                    {doc.languages.map((lang) => (
                      <span
                        key={lang}
                        className="bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700 text-slate-600 dark:text-slate-300 text-[10px] font-bold px-1.5 py-0.5 rounded"
                      >
                        {lang}
                      </span>
                    ))}
                ...
                <div className="mt-2">
                  <Button
                    asChild
                    className="w-full rounded-2xl bg-blue-600 font-bold text-xs h-10 shadow-xs shadow-blue-600/20"
                  >
