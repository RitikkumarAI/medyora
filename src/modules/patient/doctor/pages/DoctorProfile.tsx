    ...
    <div className="flex flex-col min-h-screen bg-[#F8FAFC] pb-24">
      ...
      <div className="relative h-[340px] w-full">
        ...
        <div className="absolute top-0 left-0 right-0 p-4 pt-6 flex items-center justify-between z-10">
          <Button variant="outline" size="icon" onClick={() => router.history.back()} className="h-10 w-10 rounded-full bg-white/90 backdrop-blur border-none shadow-sm text-slate-700">
            <ArrowLeft className="h-5 w-5" />
          </Button>
          <Button variant="outline" size="icon" onClick={() => setIsFavorite(!isFavorite)} className="h-10 w-10 rounded-full bg-white/90 backdrop-blur border-none shadow-sm text-slate-700">
      ...
      <main className="flex-1 -mt-8 relative z-20 bg-white rounded-t-[32px] px-6 pt-8 shadow-sm">
        ...
        <section className="mb-8">
          ...
          <p className="text-sm text-slate-500 leading-relaxed font-medium">
            {doctor.fullName} is a highly experienced {doctor.speciality} with over {doctor.experience} years of experience. 
            Dedicated to providing the best patient care and specialized in advanced treatments.
          </p>
        </section>
        ...
        <section className="mb-10 pb-10">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => setShowReviewModal(true)} className="text-sm font-bold text-blue-600">Write a Review</button>
          </div>
          <div className="space-y-4">
            {REVIEWS.map(rev => (
              <div key={rev.id} className="p-4 rounded-2xl border border-slate-100 bg-white shadow-sm">
        ...
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-900/40 backdrop-blur-sm transition-opacity">
          <div className="w-full bg-white rounded-t-[32px] shadow-2xl flex flex-col p-6">
            <div className="flex items-center justify-between mb-6">
              ...
              <Button variant="ghost" size="icon" onClick={() => setShowReviewModal(false)} className="h-8 w-8 rounded-full bg-slate-100 hover:bg-slate-200">
                <X className="h-4 w-4" />
              </Button>
            </div>
            ...
            <div className="mb-6 flex justify-center gap-2">
              {[1,2,3,4,5].map(star => (
                <button key={star} onClick={() => setRatingInput(star)} className="p-2 transition-transform active:scale-95">
                  <Star className={`h-10 w-10 ${ratingInput >= star ? "fill-amber-400 text-amber-400" : "fill-slate-100 text-slate-200"}`} />
                </button>
              ))}
            </div>
